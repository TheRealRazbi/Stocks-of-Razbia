        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url=self.validate_url_endpoint, params=querystring) as res:
                if res.status == 200:
                    ...
                    self.display_name = res_json.get("twitch", {}).get("display_name")
                    ...
                    print_with_time(f"{self.token_name} Token validation failed. It seems to be invalid... Please generate a new one on the home page of the web interface ",
                                    "red")
                    self.delete_token()
                    return False
                elif res.status >= 500:
                    raise ValueError(f"Streamlabs token servers seem to be down. This is an unrecoverable error. Error code: {res.status}")
                else:
                    raise ValueError(
